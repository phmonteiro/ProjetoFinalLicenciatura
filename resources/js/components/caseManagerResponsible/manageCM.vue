<template>
    <div>
        <set-cm :user="currentUser" :studentCMs="studentCMs" @save-user="saveUser()" @refreshCMs="getStudentCMs(currentUser)" @cancel-edit="cancelEdit()"></set-cm>
        <div class="container">
            <h2>Lista de utilizadores</h2>
            <b-table striped hover v-if="users!=null" :items="users" :fields="fields">
                <template slot="actions" slot-scope="row">
                    <button class="btn btn-secondary" v-on:click.prevent="editUser(row.item)"
                        v-if="row.item.number != user.number">Gerir</button>
                </template>
            </b-table>
            <nav aria-label="Page navigation" v-if="users">
                <ul class="pagination">
                    <li v-bind:class="[{disabled: !pagination.prev_page_url}]" class="page-item">
                        <a class="page-link" href="#" @click.prevent="getUsers(pagination.prev_page_url)">Anterior</a>
                    </li>
                    ...
                    <li v-bind:class="[{disabled: !pagination.next_page_url}]" class="page-item">
                        <a class="page-link" href="#" @click.prevent="getUsers(pagination.next_page_url)">Próximo</a>
                    </li>
                </ul>
            </nav>
        </div>
