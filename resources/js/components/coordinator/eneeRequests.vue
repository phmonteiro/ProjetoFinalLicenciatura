        ...
        <div class="container">
            ...
            <b-table striped hover v-if="requests!=null" :items="requests" :fields="fields">
                <template slot="actions" slot-scope="row">
                    ...
                    <button class="btn btn-success" v-on:click.prevent="approve(row.item.id)"
                        v-if="row.item.number != user.number">Aprovar</button>
                    <button class="btn btn-danger" v-on:click.prevent="deny(row.item.id)"
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="form-group">
                                        Nascido em: {{row.item.birthDate}}
                                    </div>

                                    <div class="form-group">
                                        Escola: {{row.item.school}}
                                    </div>

                                    <div class="form-group">
                                        Curso: {{row.item.course}}
                                    </div>

                                    <div class="form-group">
                                        Ano curricular: {{row.item.curricularYear}}
                                    </div>

                                    <div class="form-group">
                                        Entrou na escola em: {{row.item.enruledYear}}
                                    </div>

                                    <div class="form-group">
                                        Incapacidade: {{row.item.functionalAnalysis}}
                                    </div>

                                    <div class="form-group">
                                        Pedido: {{row.item.educationalSupport}}
                                    </div>
                ...
                <ul class="pagination">
                    <li v-bind:class="[{disabled: !pagination.prev_page_url}]" class="page-item">
                        <a class="page-link" href="#"
                            @click.prevent="getRequests(pagination.prev_page_url)">Anterior</a>
                    </li>
                    ...
                    <li v-bind:class="[{disabled: !pagination.next_page_url}]" class="page-item">
                        <a class="page-link" href="#" @click.prevent="getRequests(pagination.next_page_url)">Próximo</a>
                    </li>
                </ul>
            </nav>
...
<script>
    export default {
        ...
        methods: {
            ...
            approve(userId) {
                axios
                    .post("api/approveEneeStatus/" + userId)
                    .then(response => {
                        this.getRequests();
            ...
            deny(userId) {
                axios
                    .post("api/denyEneeStatus/" + userId)
                    .then(response => {
                        this.getRequests();
                        this.currentUser = null;
                        this.$toasted.success("Submetido com sucesso.", {
