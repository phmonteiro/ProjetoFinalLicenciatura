                        ...
                        <div class="form-group">
                            <div v-if="options!=null">
                                <b-form-group label="Apoios ao estudante">
                                    <b-form-radio-group v-model="service.requestedSupport" :options="options" switches>
                                    </b-form-radio-group>
                                </b-form-group>
                            </div>
                            <i v-show="errors.has('service')" class="fa fa-warning"></i>
                            <span v-show="errors.has('service')"
                                class="help is-danger">{{ errors.first('service') }}</span>
                        </div>
                        <div class="form-group">
                            <label for="comment">{{ $t('motivo') }}</label>
                            <textarea v-validate="'required'" class="form-control" id="reason" v-model="service.reason"
                                name="reason" rows="3"></textarea>  
                            <i v-show="errors.has('reason')" class="fa fa-warning"></i>
                            <span v-show="errors.has('reason')"
                                class="help is-danger">{{ errors.first('reason') }}</span>
                        </div>
