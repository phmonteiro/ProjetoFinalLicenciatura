<template>
    <div>
        <div class="container" v-if="supports">
            ...
            <edit-support :support="currentSupport" @save-support="saveSupport()" @cancel-edit="cancelEdit()">
            </edit-support>
            <div class="loader">
            ...
            <div class="container">
                ...
                <b-table striped hover v-if="supports!=null" :items="supports" :fields="fields" :per-page="perPage"
                    :current-page="currentPage">
                    <template slot="actions" slot-scope="row">
                        <button class="btn btn-secondary" v-on:click.prevent="editSupport(row.item)"
                            v-if="row.item.number != user.number">Editar</button>
                        <button class="btn btn-danger" v-on:click.prevent="deleteSupport(row.item)"
                            v-if="row.item.number != user.number">Apagar</button>
                    </template>
                </b-table>
...
<script>
    export default {
        ...
        methods: {
            ...
            saveSupport() {
                axios
                    .post("api/editSupport/" + this.currentSupport.value, this.currentSupport)
                    .then(response => {
                        this.getSupports();
                        this.currentSupport = null;
